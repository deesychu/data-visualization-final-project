...
(function() {
    ...
    function getCountsForYear(yr) { return yearlyMap.get(yr) || new Map(); }

    function makeColorScale(counts) {
        const maxVal = d3.max(Array.from(counts.values()).filter(v => v > 0)) || 1;
        // Reversed so high count → red (index 0), low count → green (index 4)
    ...
    function drawLegend(counts) {
        ...
        // Legend left = green (low), right = red (high)
    ...
    Promise.all([d3.csv("data/evictions_clean.csv"), d3.json("data/Analysis_Neighborhoods.geojson")])
        .then(([raw, geo]) => {
            ...
            const fitGeo = {
                ...
                features: geo.features.filter(f => !EXCLUDE.has(f.properties.nhood))
            };
    ...
    // Slider: 0–28 maps to 1997–2025, no "All Years"
